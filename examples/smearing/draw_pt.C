...
draw_pt()
{
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    auto g2b = lutRead_pt((std::string("lutCovm.") + name[i] + std::string(".2kG.100cm.default.dat")).c_str());
    g2b->SetLineColor(kRed+1);
    g2b->SetLineStyle(kSolid);
    g2b->SetLineWidth(3);
    g2b->Draw("samel");

    auto g5a = lutRead_pt((std::string("lutCovm.") + name[i] + std::string(".5kG.20cm.default.dat")).c_str());
    g5a->SetLineColor(kAzure-3);
    g5a->SetLineStyle(kDashed);
    g5a->SetLineWidth(3);
    g5a->Draw("samel");

    auto g5b = lutRead_pt((std::string("lutCovm.") + name[i] + std::string(".5kG.100cm.default.dat")).c_str());
    g5b->SetLineColor(kAzure-3);
    g5b->SetLineStyle(kSolid);
    g5b->SetLineWidth(3);
    g5b->Draw("samel");
    
    c->SaveAs((std::string("draw_pt.") + name[i] + std::string(".png")).c_str());

    cc->cd(i + 1);
    c->DrawClonePad();
  }

  cc->SaveAs("draw_pt.png");
