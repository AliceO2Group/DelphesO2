...
draw_eff_tof()
{
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    auto g1 = lutRead_eff((std::string("lutCovm.") + name[i] + std::string(".5kG.20cm.tof1.dat")).c_str());
    g1->SetLineColor(kRed+1);
    g1->SetLineStyle(kSolid);
    g1->SetLineWidth(3);
    g1->Draw("samel");

    auto g2 = lutRead_eff((std::string("lutCovm.") + name[i] + std::string(".5kG.20cm.tof2.dat")).c_str());
    ...
    c->SaveAs((std::string("draw_eff_tof.") + name[i] + std::string(".png")).c_str());

    cc->cd(i + 1);
    c->DrawClonePad();
  }

  cc->SaveAs("draw_eff_tof.png");
