...
draw_scenarios()
{
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    auto g1 = lutRead_dca((std::string("lutCovm.") + name[i] + std::string(".5kG.20cm.scenario1.dat")).c_str());
    g1->SetLineColor(kRed+1);
    g1->SetLineStyle(kSolid);
    g1->SetLineWidth(3);
    g1->Draw("samel");
    
    auto g2 = lutRead_dca((std::string("lutCovm.") + name[i] + std::string(".5kG.20cm.scenario2.dat")).c_str());
    g2->SetLineColor(kRed+1);
    g2->SetLineStyle(kDashed);
    g2->SetLineWidth(3);
    g2->Draw("samel");

    auto g3 = lutRead_dca((std::string("lutCovm.") + name[i] + std::string(".5kG.20cm.scenario3.dat")).c_str());
    g3->SetLineColor(kAzure-3);
    g3->SetLineStyle(kSolid);
    g3->SetLineWidth(3);
    g3->Draw("samel");

    auto g4 = lutRead_dca((std::string("lutCovm.") + name[i] + std::string(".5kG.20cm.scenario4.dat")).c_str());
    g4->SetLineColor(kAzure-3);
    g4->SetLineStyle(kDashed);
    g4->SetLineWidth(3);
    g4->Draw("samel");

    c->SaveAs((std::string("draw_scenarios.") + name[i] + std::string(".png")).c_str());
    
    cc->cd(i + 1);
    c->DrawClonePad();
  }

  cc->SaveAs("draw_scenarios.png");
