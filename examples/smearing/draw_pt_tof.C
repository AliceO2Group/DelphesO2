...
draw_pt_tof()
{
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    auto g1 = lutRead_pt((std::string("lutCovm.") + name[i] + std::string(".5kG.20cm.tof1.dat")).c_str());
    g1->SetLineColor(kRed+1);
    g1->SetLineStyle(kSolid);
    g1->SetLineWidth(3);
    g1->Draw("samel");

    auto g2 = lutRead_pt((std::string("lutCovm.") + name[i] + std::string(".5kG.20cm.tof2.dat")).c_str());
    g2->SetLineColor(kAzure-3);
    g2->SetLineStyle(kSolid);
    g2->SetLineWidth(3);
    g2->Draw("samel");

    auto gdef = lutRead_pt((std::string("lutCovm.") + name[i] + std::string(".5kG.20cm.default.dat")).c_str());
    gdef->SetLineColor(kBlack);
    gdef->SetLineStyle(kDashed);
    gdef->SetLineWidth(2);
    gdef->Draw("samel");
    
    c->SaveAs((std::string("draw_pt_tof.") + name[i] + std::string(".png")).c_str());

    cc->cd(i + 1);
    c->DrawClonePad();
  }

  cc->SaveAs("draw_pt_tof.png");
